...
test('test with default args', async () => {
  ...
  expect(output[0].startsWith("peptide,cutoff_used,total_protein_count,ipr_code,ipr_protein_count")).toBeTruthy();
...
test('test with fasta', async () => {
  ...
  expect(output[0].startsWith("fasta_header,peptide,cutoff_used,total_protein_count,ipr_code,ipr_protein_count")).toBeTruthy();
