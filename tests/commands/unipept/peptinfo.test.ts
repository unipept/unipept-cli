...
test('test with default args', async () => {
  ...
  expect(output[0].startsWith("peptide,cutoff_used,total_protein_count,taxon_id,taxon_name,taxon_rank,ec_number,ec_protein_count,go_term,go_protein_count,ipr_code,ipr_protein_count")).toBeTruthy();
...
test('test with fasta', async () => {
  ...
  expect(output[0].startsWith("fasta_header,peptide,cutoff_used,total_protein_count,taxon_id,taxon_name,taxon_rank,ec_number,ec_protein_count,go_term,go_protein_count,ipr_code,ipr_protein_count")).toBeTruthy();
